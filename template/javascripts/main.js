$ ( document ) . ready ( function ( ) {
	...
	$.post ( BOARD_AJAX + '/provider' , { 'provider' : 'Adgate' } , function ( data ) {
        $ ( '#provider' ) . html ( data ) ;
    } ) ;
	$ ( '#earn_nav ul li' ) . click ( function ( e ) {

		$.post ( BOARD_AJAX + '/provider' , { 'provider' : $ ( this ) . text ( ) } , function ( data ) {
            $ ( '#provider' ) . html ( data ) ;
        } ) ;
	...
	$ ( '#top_drop' ) . click ( function ( ) {
		$ ( '#top_container' ) . animate ( { width:'toggle' } , 150 , function ( ) {
			if ( show_top == 0 ) {
				$ ( '#top_drop' ) . css ( "left" , "150px" ) ;
				show_top = 1 ;
			}
			else {
				$ ( '#top_drop' ) . css ( "left" , "-70px" ) ;
				show_top = 0 ;
			}
		} ) ;
	} ) ;
	...
	$ ( '#referralurl' ) . click ( function ( ) {
		var content = $ ( '#referralurl_content' ) . val ( ) ;
		$.post ( BOARD_AJAX + '/referralurl' , { 'url' : content } , function ( data ) {
			$ ( '#response' ) . fadeIn ( ) ;
			setInterval ( function ( ) { $ ( '#response' ) . fadeOut ( ) ; } , 5000 ) ;
            $ ( '#response' ) . html ( data ) ;
        } ) ;
	...
	$ ( '#referralcode' ) . click ( function ( ) {
		var content = $ ( '#referralcode_content' ) . val ( ) ;
		$.post ( BOARD_AJAX + '/referralcode' , { 'url' : content } , function ( data ) {
			$ ( '#response' ) . fadeIn ( ) ;
			setInterval ( function ( ) { $ ( '#response' ) . fadeOut ( ) ; } , 5000 ) ;
            $ ( '#response' ) . html ( data ) ;
        } ) ;
