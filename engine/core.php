...
class core {
	...
	function __connect_to_mysql() {
		try {
			self::$mysql_handle = new DB(self::$global_configs['DB_TYPE'], self::$global_configs['DB_HOST'], self::$global_configs['DB_USER'], self::$global_configs['DB_PASSWORD'], self::$global_configs['DB_DB']);
	...
	function __init_folders($engine, $template, $delimiter) {
	...
	function __url_string_query() {
		if (self::$global_configs['USE_HTACCESS'] == true) $uri = substr($_SERVER['REQUEST_URI'], strlen(strstr($_SERVER['SCRIPT_NAME'], 'index.php', true)));
		else {
			$uri = substr($_SERVER['REQUEST_URI'], strlen(strstr($_SERVER['SCRIPT_NAME'], 'index.php', true)));
			$uri = substr($uri, 10);
		}

		self::$query_string = explode(self::$global_configs['SPLIT_CHAR_URL'], rtrim(ltrim(__F::__protected_string($uri) , self::$global_configs['SPLIT_CHAR_URL']) , self::$global_configs['SPLIT_CHAR_URL']));
	}
	...
	function __init_constants() {
		define('URL', '' . ((self::$global_configs['USE_HTTPS']) ? 'https' : 'http') . '://' . __F::__protected_string($_SERVER['SERVER_NAME']) . substr(__F::__protected_string($_SERVER['SCRIPT_NAME']) , 0, strpos(__F::__protected_string($_SERVER['SCRIPT_NAME']) , 'index')) . ((self::$global_configs['USE_HTACCESS']) ? '' : 'index.php?'));
		define('REAL_URL', '' . ((self::$global_configs['USE_HTTPS']) ? 'https' : 'http') . '://' . __F::__protected_string($_SERVER['SERVER_NAME']) . substr(__F::__protected_string($_SERVER['SCRIPT_NAME']) , 0, strpos(__F::__protected_string($_SERVER['SCRIPT_NAME']) , 'index')));
