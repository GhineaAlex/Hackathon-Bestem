...
class view {
    ...
    static $_user_type, $_user_information = array();
    ...
    function __render($configs = array(), $hhf = true) {
        ...
        if ($this->_main_page === 'xmlhttprequest') {
            ...
            if (file_exists(TEMPLATE_PATH . 'pages' . core::$global_configs['OS_SEPARATOR'] . $this->_main_page . '.html')) {
                if ($hhf == true)
                    $this->__html(TEMPLATE_PATH . 'hhf' . core::$global_configs['OS_SEPARATOR'] . 'header.html');
                $this->__html(TEMPLATE_PATH . 'pages' . core::$global_configs['OS_SEPARATOR'] . $this->_main_page . '.html');
                if ($hhf == true)
                    $this->__html(TEMPLATE_PATH . 'hhf' . core::$global_configs['OS_SEPARATOR'] . 'footer.html');
    ...
    static function __global_vars() {
        ...
        $results = DB::__db_query(core::$mysql_handle, DB::$DB_FETCH_ALL, DB::$DB_PROTECTED, 'SELECT `personaname`, `avatar_medium`, `coins` FROM `accounts` ORDER BY `coins` DESC LIMIT 10');
                        ...
                        if (is_array(self::$_page_array[__F::__remove_square_bracket($results[1][0])])) {
                            ...
                            foreach (self::$_page_array[__F::__remove_square_bracket($results[1][0])] as $key_array => $value_array) {
                                ...
                                if (is_array($value_array)) {
                                    ...
                                    foreach ($array_results[1] as $array_key => $array_value)
                                        $foreach_string = str_replace('[' . $foreach_value . '](' . $array_value . ')', htmlspecialchars_decode($value_array[$array_value]), $foreach_string);
                                } else
                                    $foreach_string = str_replace('[' . $foreach_key . ']', $value_array, $foreach_string);
                ...
                if (preg_match('/<\!--\s*END_IF\s*\[\s*' . $results[2][$key] . '\s*\]\s*-->/is', $html_content)) {
                    ...
                    try {
                        if (eval('return ' . $results[1][$key] . ' ;')) {
                            ...
                            if ($exist_else == false)
                                $html_content = preg_replace('/<\!--\s*END_IF\s*\[\s*' . $results[2][$key] . '\s*\]\s*-->/is', '', $html_content);
                            
                            else
                                $html_content = preg_replace('/<\!--\s*ELSE\s*\[' . $results[2][$key] . '\]\s*-->.*?<\!--\s*END_IF\s*\[' . $results[2][$key] . '\]\s*-->/is', '', $html_content);
                        } else {
                            if ($exist_else == false)
                                $html_content = preg_replace('/<\!--\s*IF\s*\(\s*' . $results[1][$key] . '\s*\)\s*\[\s*' . $results[2][$key] . '\s*\]\s*-->.*?<\!--\s*END_IF\s*\[\s*' . $results[2][$key] . '\s*\]\s*-->/is', '', $html_content);
